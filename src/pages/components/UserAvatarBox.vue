<template>
  <div class="user-avatar-box" :style="`--scale: ${scale}; ${shadow ? `box-shadow: var(--box-shadow-min);` : ''}; ${borderColor ? 'border: 2px solid ' + borderColor : ''}`">
    <div class="default-user-avatar" v-html="svg.avatar"></div>
    <div class="user-avatar"
         v-if="userAvatar"
         :style="`background-image: ${addBackground(userAvatar)}`"></div>
  </div>
</template>
...
export default {
  name: "UserAvatarBox",
  props: ["userAvatar", "scale", "shadow", "borderColor"],
  data() {
    return {
      svg: {
